...
<head>
    ...
    ob_start(); // menyimpan data pada buffer sehinnga browser tidak perlu load kembali
    session_start(); /* mengaktifkan session (data yang disimpan pada server yang dapat digunakan secara global pada 
    server tersebut dimana data itu merujuk pada klien tertentu berdasarkan session yang diaktifkan) */

    if (isset($_POST['submitLogin'])) {
        $emailUser = $_POST['userEmail'];
        $userPasswd = MD5($_POST['pass']);

        $sql_login = mysqli_query($connection, "select * from admin
        where Email_Admin = '$emailUser' and Password = '$userPasswd'");

        if (mysqli_num_rows($sql_login) > 0) {
            $row_admin = mysqli_fetch_array($sql_login);

            $_SESSION['userKode'] = $row_admin['adminID'];
            $_SESSION['userEmail'] = $row_admin['Email_Admin'];
